...
public sealed class ConversionDetails
{
    ...
    internal ConversionDetails(RawClient rawClient, Server server)
    {
    ...
    /// <exception cref="SdkException{TResult}"> of <see cref="GetConversionDetailError"/> when the server returns an error response.</exception>
    public Task GetConversionDetail(DateTimeOffset startTime,
        DateTimeOffset endTime,
        string? organizationId,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default("/reporting/v3/conversion-details"),
            [],
            ...
            GetConversionDetailErrorResponse.Instance,
            [],
            requestOptions,
            ct);
}
