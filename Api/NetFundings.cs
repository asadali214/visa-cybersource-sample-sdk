...
/// </summary>
public sealed class NetFundings
{
    private readonly RawClient _rawClient;
    private readonly Server _server;

    internal NetFundings(RawClient rawClient, Server server)
    {
    ...
    /// <param name="groupName">Valid CyberSource Group Name.</param>
    /// <param name="requestOptions">Per-request options, such as an overriding log level for this call</param>
    /// <param name="ct">Cancellation token</param>
    /// <returns>A <see cref="Task"/> instance.</returns>
    /// <exception cref="SdkException{TResult}"> of <see cref="GetNetFundingDetailsError"/> when the server returns an error response.</exception>
    public Task GetNetFundingDetails(DateTimeOffset startTime,
        DateTimeOffset endTime,
        string? organizationId,
        string? groupName,
        RequestOptions? requestOptions = null,
        CancellationToken ct = default) =>
        _rawClient.Execute(_server.Default("/reporting/v3/net-fundings"),
            [],
            ...
            GetNetFundingDetailsErrorResponse.Instance,
            [],
            requestOptions,
            ct);
}
