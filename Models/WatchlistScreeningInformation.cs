...
public record WatchlistScreeningInformation
{
    /// <summary>
    ...
    /// Possible values:
    /// - <c>true</c>: Transaction proceeds even when match is found in the Denied Parties List. The match is noted in the response.
    /// - <c>false</c>: Normal watchlist screening behavior occurs. (Transaction stops if a match to DPL occurs. Transaction proceeds if no match.)
    /// </summary>
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    [JsonPropertyName("proceedOnMatch")]
    public bool? ProceedOnMatch { get; init; }
