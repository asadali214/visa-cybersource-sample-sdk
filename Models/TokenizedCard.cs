...
public record TokenizedCard
{
    ...
    /// <para>
    /// Possible value:
    /// - <c>2</c>: Near-field communication (NFC) transaction. The customer’s mobile device provided the token data for a contactless EMV transaction. For recurring
    /// transactions, use this value if the original transaction was a contactless EMV transaction.
    /// </para>
    /// <para>
    /// #### Visa Platform Connect
    /// - <c>1</c>: For Rupay and In App tokenization. Example: InApp apple pay.
    /// - <c>3</c>: Card/Credential On File Tokenization.
    /// </para>
    /// <para>
    ...
    /// #### Rupay
    /// - <c>3</c>: Card/Credential On File Tokenization.
    /// - <c>4</c>: Tokenizined Transaction. Should be used for Guest Checkout transactions with token.
    /// </para>
    /// </summary>
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    ...
    /// <para>
    /// - <c>001</c>: Secure Element (SE). Smart card or memory with restricted access and encryption to prevent data tampering. For storing payment
    ///    credentials, a SE is tested against a set of requirements defined by the payment networks.
    /// </para>
    /// <para>
    ///    <b>Note</b> This field is supported only for _FDC Compass_.
    /// </para>
    /// <para>
    ...
    [MaxLength(3)]
    public string? StorageMethod { get; init; }

    /// <summary>
    /// Card Verification Number (CVN).
    /// <para>
    /// #### Ingenico ePayments
    /// Do not include this field when <b>commerceIndicator=recurring</b>.
    ...
    /// <para>
    ///  - <c>0</c> (default): CVN service not requested. This default value is used when you do not include
    ///      <c>securityCode</c> field in the request.
    ///  - <c>1</c> (default): CVN service requested and supported. This default value is used when you include
    ///      <c>securityCode</c> field in the request.
    ///  - <c>2</c>: CVN on credit card is illegible.
    ///  - <c>9</c>: CVN was not imprinted on credit card.
    /// </para>
    /// <para>
    /// #### FDMS Nashville
    ...
    /// Optional if <c>pointOfSaleInformation.entryMode=keyed</c>; otherwise, not used.
    /// </para>
    /// <para>
    ...
    [MaxLength(1)]
    public string? SecurityCodeIndicator { get; init; }
